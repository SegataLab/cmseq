...
def polymut_from_file(args):
	...
	for i in tl:
		...
		outputDicts.append({'DN':mutationStats['DN'],'DS':mutationStats['DS'],'D?':mutationStats['D?'], "consid_pos":len([x for x in dominanceArray if x  != np.nan])})
